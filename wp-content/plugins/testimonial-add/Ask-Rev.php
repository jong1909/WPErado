...
function plugTSS_check_installation_date() {
 
    $nobug = "";
    $nobug = get_option('plugTSS_hide_noticeSec');
 
    if (!$nobug) {
        add_action( 'admin_notices', 'plugTSS_display_admin_notice' );
    }
}
add_action( 'admin_init', 'plugTSS_check_installation_date' );
 
function plugTSS_display_admin_notice() {
    ...
    echo '<div style="width:65%; display: inline-block;" ><br> <h3><b>' . __( 'Do You Like Testimonial Slider ? You\'ll Love PluginOps Page Builder! ', 'page-builder-add' ) . '</b></h3>';

    echo '<p style="font-size:16px;">' . __( ' <i> Tip :  </i> Create responsive pages & landing pages with drag and drop page builder by pluginOps, Create customizable pages   So Install the page builder now Its <b>Free!!</b> ', 'page-builder-add' ) . '</p>
...
function plugTSS_set_no_bug() {
    ...
    if ( 1 == $nobug ) {
        ...
        add_option( 'plugTSS_hide_noticeSec', TRUE );
    }
 
} add_action( 'admin_init', 'plugTSS_set_no_bug', 5 );
