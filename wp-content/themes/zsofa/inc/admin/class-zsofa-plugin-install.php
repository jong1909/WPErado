...
if ( ! class_exists( 'zsofa_Plugin_Install' ) ) :
	/**
	 * The zsofa plugin install class
	 */
	class zsofa_Plugin_Install {
		...
		public function plugin_install_scripts( $hook_suffix ) {
			...
			wp_enqueue_script( 'zsofa-plugin-install', get_template_directory_uri() . '/assets/js/admin/plugin-install' . $suffix . '.js', array( 'jquery', 'updates' ), $zsofa_version, 'all' );

			wp_enqueue_style( 'zsofa-plugin-install', get_template_directory_uri() . '/assets/css/admin/plugin-install.css', array(), $zsofa_version, 'all' );
		}
			...
			if ( current_user_can( 'install_plugins' ) && current_user_can( 'activate_plugins' ) ) {
				if ( is_plugin_active( $plugin_slug . '/' . $plugin_file ) ) {
					...
					$button = array(
						...
						'url'     => '#',
						'classes' => array( 'zsofa-button', 'disabled' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'zsofa-button', 'activate-now' ),
					);
					...
					$button = array(
						...
						'url'     => $url,
						'classes' => array( 'zsofa-button', 'sf-install-now', 'install-now', 'install-' . $plugin_slug ),
					);
...
return new zsofa_Plugin_Install();
