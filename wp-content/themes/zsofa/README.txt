...
zsofa is the perfect theme for your next WooCommerce project.

== Description ==

zsofa is the perfect theme for your next WooCommerce project. Designed and developed by WooCommerce Core developers, it features a bespoke integration with WooCommerce itself plus many of the most popular customer facing WooCommerce extensions. There are several layout & color options to personalize your shop, multiple widget regions, a responsive design and much more. Developers will love its lean and extensible codebase making it a joy to customize and extend. Looking for a WooCommerce theme? Look no further!

For more information about zsofa please go to https://woocommerce.com/zsofa/.

For even more customization, check out zsofa extensions https://woocommerce.com/product-category/zsofa-extensions/ and zsofa child themes https://woocommerce.com/product-category/themes/zsofa-child-theme-themes/.

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in zsofa in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Go to https://docs.woocommerce.com/documentation/themes/zsofa/ guides on how to customize this theme.
...
zsofa is based on Underscores https://underscores.me/, (C) 2012-2017 Automattic, Inc.
...
zsofa serves fonts via the hosted Google Fonts service. zsofa does not share any data with Google directly.
...
* Fix - Moved margin from `.woocommerce-breadcrumb` to the new `.zsofa-breadcrumb` wrapper.
...
= 2.2.7 - 2018-01-23 =
* Fix - Reverted Chromium overflow fix introduced in 2.2.6 due to incompatibility with other zsofa products.
* Fix - Jetpack Google Translate widget styling.
* Tweak - Improved Grouped products table styling.
* Dev - Added filters; `zsofa_custom_logo_args`, `zsofa_register_nav_menus`, `zsofa_html5_args`, and `zsofa_site_logo_args`.
...
* Fix - Fixed typo in the `zsofa_WooCommerce` class, `thumbnail_columns` method description. Kudos [@cryptexvinci](https://github.com/cryptexvinci).
...
* Dev - Removed sticky cart. See [this issue](https://github.com/woocommerce/zsofa/issues/637) for more information.
...
* Dev - New hook to filter sidebar regions: `zsofa_sidebar_args`.
...
* Tweak - Removed Structured Data. See [this comment](https://github.com/woocommerce/zsofa/issues/620#issuecomment-313629032).
...
* Dev - New hook after body tag: `zsofa_before_site`.
...
* Dev - Added actions; `zsofa_jetpack_infinite_scroll_before`, `zsofa_jetpack_infinite_scroll_after`, `zsofa_jetpack_product_infinite_scroll_before` and `zsofa_jetpack_product_infinite_scroll_after`.
* Dev - Added filters; `zsofa_footer_widget_rows` and `zsofa_footer_widget_columns` to easily control the number of widget rows / columns in the footer.
...
* Dev - Upsells columns filterable via `zsofa_upsells_columns`.
* Dev - Added a column wrapper to product loops. To adjust the layout you now only need to filter `zsofa_loop_columns`.
...
* Dev - Refactored `zsofa_get_content_background_color()` to account for and give priority (over zsofa Designer) to zsofa Powerpack.
* Dev - Deprecated `is_woocommerce_activated()` and made it pluggable.
* Dev - Added `zsofa_is_woocommerce_activated()`.
...
* Dev - `zsofa_post_thumbnail()` is now hooked in to `zsofa_post_content_before`.
* Dev - Added `zsofa_post_content_before` and `zsofa_post_content_after` actions.
* Dev - `zsofa_post_thumbnail()` now provides a default size.
...
* Dev - Restructured the way objects are initiated. Objects are now initiated into an accessible global $zsofa for access to hooks. Kudos [@jtsternberg](https://github.com/jtsternberg).
...
* Dev - Made `zsofa_primary_navigation_wrapper()` and `zsofa_primary_navigation_wrapper_close()` pluggable.
...
* Dev - Deprecate `zsofa_html_tag_schema()`.

= 2.0.3 - 2016-06-02 =
* Fix - Fatal error when using zsofa without WooCommerce.
...
* Dev - Added `zsofa_sticky_order_review` filter.
* Dev - Added `zsofa_structured_data` filter for structured data customisation.
...
* Dev - Apply Customizer setting defaults on `customize_register` to account for child themes filtering `zsofa_setting_default_values`.
* Dev - Fix typo in the `zsofa_homepage_after_best_selling_products` action name.
...
* Feature - oEmbed styling to match zsofa aesthetic.
* Feature - The welcome screen now dynamically pulls in new products.
* Feature - Added `zsofa_header_styles()` for the header image/styles along with the `zsofa_header_styles` filter to customise the treatment of header styles.
...
* Dev - Deprecated `zsofa_categorized_blog()`.
* Dev - Deprecated `zsofa_sanitize_hex_color()`.
* Dev - Deprecated `zsofa_sanitize_layout()`.
...
= 1.6.0 - 2016-01-05 =
* Feature - [zsofa Mega Menus](https://woocommerce.com/products/zsofa-mega-menus/) integration.
...
* Tweak - Added new function `zsofa_star_rating_script`. Outputs JavaScript for the new star rating input while we wait for WooCommerce 2.5 to be released.
...
* Dev - Added some new hooks to the homepage template tags; `zsofa_homepage_after_product_categories_title`, `zsofa_homepage_after_recent_products_title`, `zsofa_homepage_after_featured_products_title`, `zsofa_homepage_after_popular_products_title`, `zsofa_homepage_after_on_sale_products_title`
* Dev - Styling for 6 column product layouts.
* Dev - Updates FontAwesome to version 4.5.0.

= 1.5.3 - 2015-11-13 =
* Fix - zsofa will now automatically load a child themes stylesheet.
...
* Dev - `zsofa_display_comments` is now hooked into `zsofa_single_post_after` with priority 20 (was 10).
* Localization - All translations are now managed on [WordPress.org](https://translate.wordpress.org/projects/wp-themes/zsofa)
...
* Dev - The default layout is now  filterable via `zsofa_default_layout`.
...
* Dev - Added `zsofa_sanitize_checkbox()` sanitization function.
* Dev - Added `zsofa_Custom_Radio_Image_Control` class for creating radio image controls in the Customizer.
* Dev - Added `zsofa_post_thumbnail()`.
* Dev - Renamed `do_shortcode_func()` to `zsofa_do_shortcode()`.
...
* Dev - `$zsofa_version` global when using a child theme.

= 1.4.5 - 2015-05-14 =
* Fix - Use the correct escaping function in `zsofa_product_categories()`.
...
* Dev - Made `Layout_Picker_zsofa_Control` class pluggable. Props @niravmehta
...
* Tweak - Skip links are now a function (`zsofa_skip_links()`) hooked into `zsofa_header`.
...
* Dev - Fixed typo in classname: `zsofa-feautred-products` is now `zsofa-featured-products`.
...
* Dev - Added `zsofa_related_products_args` filter.
...
* Fix - `zsofa_menu_toggle_text` filter. Props [jesinwp](https://github.com/jesinwp).
...
* Tweak - `mark` styling.
* Dev - Added `zsofa_sanitize_choices()`.
...
* Dev - `zsofa_header_cart()` is now pluggable.
...
* Dev - Added `zsofa_menu_toggle_text` filter.
* Dev - Updated how WooCommerce styles are dequeued.

= 1.2.3 - 2015-01-07 =
* Fix - Button border in WooCommerce messages when using the zsofa Designer extension.
* Tweak - Stock icon.
...
* Dev - Made `zsofa_product_search()` pluggable.
...
* Dev - Added `zsofa_single_post_posted_on_html` filter.
...
* Dev - Added `zsofa_copyright_text` filter.
* Dev - Tweaked how zsofa determines whether Customizer logic should be loaded.
* Dev - Added `zsofa_after_footer` action.
...
* Dev - Added `zsofa_product_thumbnail_columns` filter.
