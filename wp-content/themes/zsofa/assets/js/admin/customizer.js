...
( function( wp, $ ) {
	...
	api.SFGuidedTour = {
		...
		_hideTour: function( remove ) {
			...
			$( 'body' ).addClass( 'sf-exiting' ).on( 'animationend.zsofa webkitAnimationEnd.zsofa', function() {
				$( this ).removeClass( 'sf-exiting' ).off( 'animationend.zsofa webkitAnimationEnd.zsofa' ).addClass( 'sf-hidden' );
				self.$container.hide();
		...
		_revealTour: function() {
			...
			$( 'body' ).addClass( 'sf-entering' ).on( 'animationend.zsofa webkitAnimationEnd.zsofa', function() {
				$( this ).removeClass( 'sf-entering' ).off( 'animationend.zsofa webkitAnimationEnd.zsofa' );
		...
		_moveContainer: function( $selector ) {
			...
			this.$container.addClass( 'sf-moving' ).css({ 'transform': 'translateY(' + parseInt( position, 10 ) + 'px)' }).on( 'transitionend.zsofa', function() {
				self.$container.removeClass( 'sf-moving' );
				self.$container.off( 'transitionend.zsofa' );
			} );
		},
