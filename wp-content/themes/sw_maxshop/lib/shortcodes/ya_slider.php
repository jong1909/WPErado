...
class Ya_Slider_Shortcodes{
	...
	public function ya_woocommerce_slider( $atts, $content ){
		...
		if( $terms == NULL ){
			echo esc_html__( 'Please select a category!', 'yatheme' );
			return ;
		}
		$html = '';
		...
		/*//////////////////////////////////  end layout child cate ////////////////////////////////////////////////////
		///////////////////////////////////   layout child cate left //////////////////////////////////////////////*/
        if( $layout =='child-cate-left') {
			...
			$html.='<div class="product-childcat">
						...
						foreach ( $termchildren as $child ) {
									...
									//print '<pre>';var_dump($child); print '</pre>';
								$html .='<li class="item"><a href="' . get_term_link( intval( $child->term_id ), 'product_cat' ) . '">' . esc_html( $child->name ). '</a></li>';
								}
								$html .='<li class="item"><a class="category-show-more" href="'.get_term_link($term_id, 'product_cat').'">'.__('View more...','yatheme').'</a></li>';
			$html.='</ul>';
			$html.='</div>
			</div>';
			}			
				...
				<div class="resp-slider-container">
					...
					$html .='
					  ...
					  $html .= '<div class="item-img">';
									$html ='<a href="' .get_permalink( $post->ID ) .'" title="'. esc_attr( $post->post_title ) .'">'.get_the_post_thumnail( $post->ID ). '</a>';									
					  $html .= '</div>
								<div class="item-content">';
							$html .= '<h4><a href="' .get_permalink( $post->ID ) .'" title="'. esc_attr( $post->post_title ) .'">'. get_the_title( $post->ID ) .'</a></h4>';											
