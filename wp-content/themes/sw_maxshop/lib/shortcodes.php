...
class YA_Shortcodes{
	...
	protected $tags = array( 'icon','youtube video', 'button', 'alert', 'bloginfo', 'colorset', 'slideshow', 'googlemaps', 'columns', 'row', 'col', 'code', 'breadcrumb', 'pricing','tooltip','modal','gallery_image');
...
$output.='<a class="view-all-brand" href="#" title="View All">'.__("View All","yatheme").'</a>
		</div>';
...
if($type == '2_column'){
	...
	$output .='<span class="entry-comment"><i class="fa fa-comment"></i>'.$post->comment_count .'<span>'. __(' comments', 'yatheme').'</span></span>';
...
if($type == 'middle_right'){
	...
	$output.='<ul>';
     foreach ($list as $key => $post){
	if ( $key == 0 ) {
	$output.='<div class="view-all"><a href="'.get_category_link($category_id).'">'. esc_attr__( 'View All', 'yatheme' ).'<i class="fa fa-caret-right"></i></a></div>';
